...
class VetResourceIT {
    ...
    void createVet() throws Exception {
        ...
        restVetMockMvc
            .perform(
                post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(vet))
            )
    ...
    void putNonExistingVet() throws Exception {
        ...
        restVetMockMvc
            .perform(
                put(ENTITY_API_URL_ID, vet.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    void putWithMissingIdPathParamVet() throws Exception {
        ...
        restVetMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(vet))
            )
